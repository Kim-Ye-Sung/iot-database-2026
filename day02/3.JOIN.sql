...
SELECT c.*
 ...
 -- Customer 테이블과 Book 테이블간의 조인은 불가능
 -- 중간 매개체가 Orders 테이블
 ...
 -- 고객으로 회원가입을 해놓고 책을 한번도 구매하지 않은 사람을 조회
 SELECT *
   FROM Customer c
   LEFT OUTER JOIN Orders o
     ON c.custid = o.custid
...
-- 책 중에서 한번도 팔리지 않은 책정보도 같이 조회하시오
 SELECT *
   FROM Orders o RIGHT OUTER JOIN Book b
     ON o.bookid = b.bookid;

-- 최종적으로 book이 기준되어서 일치하지 않는 주문과 고객이 NULL로 표시
SELECT *
...
-- 최종적으로 Customer를 기준으로 일치않는 주문과 책을 NULL로 표시
SELECT *
